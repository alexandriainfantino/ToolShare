...
def is_shed_coordinator(shed, profile):
    ...
    """
    return profile in shed.shed_coordinators.all()
...
def add_admin(profile):
    """
    Adds an admin

    Args:
        profile: person to be promoted
    """
    newAdmin = profile.user.is_staff = True
    newAdmin.save()

def remove_admin(profile):
    """
    Removes an admin
    ...
    Args:
        profile: person to be demoted
    """
    oldAdmin = profile.user.is_staff = False
    oldAdmin.save()
